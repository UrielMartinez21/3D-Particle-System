...
// Elementos UI que ya no existen:
...
const material = new THREE.PointsMaterial({
  color: hexToColor("#7c5cff"), // Color fijo púrpura
  size: 0.03,
...
baseTemplate = generateTemplate('galaxy', COUNT); // Empezar con galaxy
targetTemplate = baseTemplate.slice();

// ─────────────────────────────────────────────────────────────
// Hand tracking (MediaPipe) - Sistema de gestos
// ─────────────────────────────────────────────────────────────
...
let currentGestureShape = 'galaxy';
...
function isClosedFist(landmarks) {
  ...
  // Verificar si las puntas de los dedos están cerca de la palma
  const palm = landmarks[0]; // Muñeca como referencia de palma
  const fingerTips = [4, 8, 12, 16, 20]; // Puntas de todos los dedos
  
  let closedCount = 0;
  fingerTips.forEach(tip => {
    const fingerTip = landmarks[tip];
    const distance = Math.hypot(fingerTip.x - palm.x, fingerTip.y - palm.y);
    if (distance < 0.15) { // Distancia pequeña = dedo cerrado
      closedCount++;
    }
  });
  
  return closedCount >= 4; // Al menos 4 dedos cerrados
}

// Función para detectar el símbolo de paz (dedos índice y medio extendidos)
function isPeaceSign(landmarks) {
  ...
  // MCPs (base de los dedos)
  const indexMcp = landmarks[5];
  const middleMcp = landmarks[9];
  const ringMcp = landmarks[13];
  const pinkyMcp = landmarks[17];
  
  // Verificar que índice y medio están extendidos
  const indexExtended = indexTip.y < indexMcp.y - 0.05;
  const middleExtended = middleTip.y < middleMcp.y - 0.05;
  
  // Verificar que anular y meñique están doblados
  const ringClosed = ringTip.y > ringMcp.y;
  const pinkyClosed = pinkyTip.y > pinkyMcp.y;
  
  // Verificar separación entre índice y medio
  const separation = Math.hypot(indexTip.x - middleTip.x, indexTip.y - middleTip.y);
  
  return indexExtended && middleExtended && ringClosed && pinkyClosed && separation > 0.05;
}

// Función para detectar mano abierta
function isOpenHand(landmarks) {
  if (!landmarks || landmarks.length < 21) return false;
  
  // Verificar que todos los dedos están extendidos
  const fingerTips = [4, 8, 12, 16, 20];
  const fingerMcps = [2, 5, 9, 13, 17]; // Bases aproximadas
  ...
  for (let i = 0; i < fingerTips.length; i++) {
    ...
    // Para el pulgar la lógica es diferente (movimiento lateral)
    if (i === 0) {
      const thumbBase = landmarks[1];
      const thumbDistance = Math.hypot(tip.x - thumbBase.x, tip.y - thumbBase.y);
      if (thumbDistance > 0.08) extendedCount++;
    } else {
      // Para otros dedos, verificar extensión vertical
      if (tip.y < mcp.y - 0.03) extendedCount++;
    }
  }
...
// Función principal de detección de gestos
function detectGesture(landmarks) {
  ...
  if (isClosedFist(landmarks)) {
    return { type: 'fist', shape: 'saturn', name: 'Puño cerrado' };
  } else if (isPeaceSign(landmarks)) {
    return { type: 'peace', shape: 'heart', name: 'Paz y amor' };
  } else if (isOpenHand(landmarks)) {
    return { type: 'open', shape: 'galaxy', name: 'Mano abierta' };
  }
...
function onHandResults(results) {
  if (!results.multiHandLandmarks || results.multiHandLandmarks.length === 0) {
    ...
    gestureType.textContent = "Ninguno";
  ...
  // Detectar gesto específico
  const detectedGesture = detectGesture(lm);
  
  if (detectedGesture && detectedGesture.shape !== currentGestureShape) {
    // Cambiar forma basada en el gesto
    currentGestureShape = detectedGesture.shape;
    requestShape(detectedGesture.shape);
    gestureDetectionCooldown = 30; // Cooldown de 30 frames para evitar cambios rápidos
    
    // Actualizar UI
    gestureType.textContent = detectedGesture.name;
    currentShape.textContent = detectedGesture.shape.charAt(0).toUpperCase() + detectedGesture.shape.slice(1);
  } else if (!detectedGesture) {
    gestureType.textContent = "Movimiento";
  }

  // Usar la muñeca para la posición de la mano (movimiento de la figura)
  const wrist = lm[0];
  
  // Convertir coordenadas de MediaPipe a offset de mundo
  const hx = (wrist.x - 0.5) * 4.0;     // Rango más amplio para movimiento
  const hy = (0.5 - wrist.y) * 3.0;     // Invertir Y para que arriba sea positivo
  handOffset.set(hx, hy, 0);

  handStatus.textContent = `Mano: ${Math.round(wrist.x * 100)}, ${Math.round(wrist.y * 100)}`;
}
...
let currentShapeName = 'galaxy';

function rebuild(count) {
  COUNT = count;

  positions = new Float32Array(COUNT * 3);
  baseTemplate = generateTemplate(currentShapeName, COUNT);
  targetTemplate = baseTemplate.slice();
...
// Los controles de selección manual de forma fueron removidos
...
function animate() {
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    // Sin expansión radial - mantener forma base
    ...
    // Desplazamiento por mano - más fuerte y directo
    const handPower = hasHand ? 1.2 : 0.1;
    x += handOffsetSm.x * drift * handPower;
    y += handOffsetSm.y * drift * handPower;
    z += handOffsetSm.z * drift * handPower;
